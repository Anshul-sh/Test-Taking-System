...
from SchoolManagement.models import Student, UserManager, Teacher, NotificationTeacher
...
from .models import MCQuestion , Answer ,Quiz, Course, Progress, Sitting, Question, Subject
from .forms import *
...
def createCourse(request):
    ...
    if request.method == "POST":
        ...
        Course.objects.create(Course = c)
        return redirect('indexf')

def createSubject(request):
    if request.method == 'POST':
        form1 = SubjectForm(request.POST)
        if form1.is_valid():
            form1.save()
            return  redirect(indexf)
    else:
        form1 = SubjectForm()
    return render(request, 'createsubCourse.html', {'form1': form1})
...
def importquecsv(request):
    ...
    for index, column in df.iterrows() :
        a = Question(content = str(column[0]) , explanation = str(column[1]) , Course_id = 4)
        a.save()
    return redirect(indexf)
...
def staff_notification(request):
    ...
    quiz = Quiz.objects.get(url=q_split[1])
    user = request.user.username
    staff= Teacher.objects.get(course = quiz.Course_id)
    notify = NotificationTeacher.objects.create(message= user +" is suspected for plagiarism.",staff_id=staff)
    html = '<html><body>Notification send.</body></html>'
